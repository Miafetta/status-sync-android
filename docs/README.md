...
- Enable private mode, replacing uploaded status values with `none`.
...
Settings are applied locally and are not uploaded as JSON fields. When private mode is enabled, uploaded status values are replaced with `none`.
